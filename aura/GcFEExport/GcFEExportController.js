({
	...
	handleStartExport: function(component, event, helper) {
 
        // Create and call the background action
        var action = component.get("c.exportTransactionSummaries");
		action.setParams({"transactionSummaryHeaderId": component.get("v.recordId")});
        ...
        action.setCallback(this, function(response) {
            ...
            if(component.isValid() && state === "SUCCESS") {
                ...
                console.log('Problem creating export for summary header, response state: ' + state);
            }
        });
        $A.enqueueAction(action);        
		...
		resultsToast.setParams({
			"title": "Complete",
            "message": "The file is being created : " 
		});
		resultsToast.fire();
